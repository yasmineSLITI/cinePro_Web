...
class DemandedesponsoringController extends Controller
{
    ...
    public function affiche_demande_par_id_event($id, Request $request): Response
    {
        ...
        return $this->render(
            "demandedesponsoring/afficherDemandedesponsoring.html.twig",
            ['demande' => $demandes]
        );
   ...
   $progret_actuel += $prg;*/
    ...
    public function envoyer_demande(Request $req, $num): Response
    {
        ...
        return $this->render(
            'demandedesponsoring/ajouterdemande.html.twig',
    ...
    public function affiche_demande_par_id_event_pr_admin_JSON($id, NormalizerInterface $nrm)
    {
        ...
        $qb = $this->getDoctrine()->getManager()->createQuery('select s from App\Entity\Demandedesponsoring s where s.idev = :v1')->setParameter('v1', $id);
        $demande = $qb->getArrayResult();
        $response = new Response(json_encode($demande));
        return $response;
    }
    /**
     *  
   ...
   $progret_actuel += $prg;*/
        ...
        $qb = $this->getDoctrine()->getManager()->createQuery('select s from App\Entity\Demandedesponsoring s where s.iddemande = :v1')->setParameter('v1', $demande->getIddemande());
        $demande = $qb->getArrayResult();
        $response = new Response(json_encode($demande));
        return $response;
    }
    ...
    public function affiche_des_demandes_par_spons_JSON(DemandedesponsoringRepositoy $repo, NormalizerInterface $nrm, $id)
    {
        ...
        $qb = $this->getDoctrine()->getManager()->createQuery('select s from App\Entity\Demandedesponsoring s where s.idsp = :v1')->setParameter('v1', $id);
        $demande = $qb->getArrayResult();
        $response = new Response(json_encode($demande));
        return $response;
    }
