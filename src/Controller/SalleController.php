...
/**
     ...
     */
    public function afficheJSON(NormalizerInterface $ner) {
        $repo = $this->getDoctrine()->getRepository(Salle :: class);
        $salle = $repo->findAll();
        $jsonContent = $ner->normalize($salle , 'json', ['groups'=>'salle']);
        return $this->render('JSONA/salle.html.twig',[
            'data'=> $jsonContent
        ]);
    ...
    public function ajouterJSON(Request $req , NormalizerInterface $nrm) : Response{
       ...
       $em->persist($salle);
        $em->flush();
        $jsonContent = $nrm->normalize($salle , 'json' , ['groups'=>'salle']);
        return new Response(json_encode($jsonContent));
     }
      /**
   *  
